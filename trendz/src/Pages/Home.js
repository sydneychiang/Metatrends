...
function Home() {
    ...
    const displaySearchData = (data) => {
        ...
        if (data.length!==0) {
            ...
            if (data[ls].data.length === 0) {
                elements.push(<h1 className="noResult">  There are no results for that search...</h1>)
            }
            for (let i = 0; i <data[ls].data.length; i++) {
    ...
    return (
        <div >
            ...
            <div>
                {filterObject.searchLength !== 0 ? returnButton(filterObject.searchData): returnButton(data)}
            </div>
            <div>
